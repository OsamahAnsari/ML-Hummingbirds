...
public class HummingbirdAgent : Agent
{
    ...
    [Tooltip("Whether to spawn in front of flower; only applies in game mode")]
    public bool spawnInFrontOfFlower = true;
    ...
    // Maximum distance from the beak tip to accept nect5ar collision
    public float BEAK_TIP_RADIUS = 0.1f;
    ...
    public override void OnEpisodeBegin()
    {
        ...
        bool inFrontOfFlower = spawnInFrontOfFlower;
    ...
    /// <summary>
    /// Called when an action is received from the player input or the nueral network
    /// 
    ...
    {
        // If nearestFlower is null, observe an empty array and return array
        ...
        // Observe a dot product that indicates whether a beak tip is in front of the flower (1 observation)
    ...
    /// <summary>
    /// When Behavior Type is set to "Hueristic Only" on the agent's Behavior Parameters, this function will be called.
    /// Its return value will be fed into <see cref="OnActionReceived(float[])"/> instead of using the neural network
    /// </summary>
    ...
    private void MoveToSafeRandomPosition(bool inFrontOfFlower)
    {
        ...
        Quaternion potentionalRotation = new Quaternion();
        ...
        while(!safePositionFound && attemptsRemaining > 0)
        {
            ...
            if (inFrontOfFlower)
            {
                ...
                potentionalRotation = Quaternion.LookRotation(toFlower, Vector3.up);
            }
            else
            {
                // Pick a random height from the ground
                float height = UnityEngine.Random.Range(3f, 4f);
                ...
                // Choose and set random starting pitch and ya
                float pitch = UnityEngine.Random.Range(-60f, 60f);
                float yaw = UnityEngine.Random.Range(-180f, 180f);
                potentionalRotation = Quaternion.Euler(pitch, yaw, 0f);
            }
        ...
        transform.rotation = potentionalRotation;
    }
    ...
    private void TriggerEnterOrStay(Collider collider)
    {
        ...
        if (collider.CompareTag("nectar"))
        {
            ...
            if (Vector3.Distance(beakTip.position, closestPointToBeakTip) < BEAK_TIP_RADIUS)
            {
                ...
                float nectarReceived = flower.Feed(0.25f);
